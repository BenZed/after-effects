...
function executeAppleScriptSync(scriptUrl, resultUrl) {
  ...
  try {
    ...
    const results = _fsExtra2.default.readJsonSync(resultUrl);
    return parseResults(results);
  } catch (err) {
    throw new _errors.NoResultError();
  }
}
...
async function executeAppleScript(scriptUrl, resultUrl) {
  ...
  try {
    ...
    const results = await _fsExtra2.default.readJson(resultUrl);
    return parseResults(results);
  } catch (err) {
    throw new _errors.NoResultError();
  }
...
async function execute(source, ...args) {
  ...
  const { adobified, resultPath } = (0, _transpile.adobify)(this.options, source, ...args);
  const { programDir, renderEngine } = this.options;

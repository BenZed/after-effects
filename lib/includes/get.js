...
if ($.global.is === undefined) {
	$.global.is = (function() {
		return function is(value, type) {

			const ZeroArgumentsError = new Error("is() expects at least one value and optionally a variable number of type arguments");
			const IncorrectTypeError = new Error("if a type is supplied, it is expected to be a non-anonymous Function definition.");
			const IncorrectTypesError = new Error("types, if supplied, are expected to be non-anonymous Function definitions.");

			var value_type = typeof value;

			//Validate value argument
			if (arguments.length == 0)
				throw ZeroArgumentsError;
			...
			for (var i = 1; i < arguments.length; i++)
				if (typeof type !== "function" || type.name.length === 0)
					throw arguments.length == 1 ? IncorrectTypeError : IncorrectTypesError;
	...
	$.global.get = (function(is){
		...
		function create_selector(arg)
		{
			...
			if (!is(selector, Function)) {
				selector = function(input) { return true; }
			}
		...
		function properties_to_array(group, types, arr) {

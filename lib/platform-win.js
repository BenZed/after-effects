...
function create_jsx_script(command, sync) {
  ...
  const program_path = command.options.program
    ...
    // do debug concurrent ? 
    debug.debugScript(command,jsx_script, jsx_script_file)
  if (sync) {
    try {
...
function prepareCmd(jsx,command){

  let cmd = ["afterfx.exe"] ; // , "-r" , jsx.script]; 
  
	if(command.options.multi == true ){
	  cmd.push("-m")
	}
	if(command.options.noui == true ){
	  cmd.push("-noui")
	}
   
  cmd = cmd.concat(["-r",jsx.script])
...
module.exports = {
  ...
  execute : function(command) {
    ...
    .then( jsx => new Promise( resolve => {
      if(command.options.debug == true) { 
        ...
        jsx.script = path.join(process.cwd(),`ae-command-${uuid.v4()}.jsx.log`);
        console.log("script and program path for debuging ")
        console.log(jsx) 
      }
      let cmd=  prepareCmd(jsx,command)
      exec(cmd , {cwd: jsx.program_path}, (/*err, stdout, stderr*/) => {
        // see * below
  ...
  executeSync : function(command) {
    ...
    try {
      ...
      let cmd=  prepareCmd(jsx,command)
      execSync(cmd, {cwd: jsx.program_path});
    } catch (err) {
      //TODO *

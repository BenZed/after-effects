...
function stringify() { /* private dangler */
	...
	let inc = this.includes.join(';');
	let main = err_open + res_get + this.code + '.apply(this' + args + ');\n' + err_close + res_write;
...
function codify_includes(includes, do_minify) {
	...
	return includes.map(file => {
		...
		try {
			...
			throw(`Cannot include file at path ${file}, non-existent or can't be read.`);
		}
...
module.exports = class Command {
	...
	constructor(func, options) {
		...
		//Pass function as an expression otherwise it breaks when isolated
		this.code = babelify(`(${func.toString()})`);

		//Isolate babelified code to just the funciton expression (remove 'use strict' and final ';')
		this.code = this.code.substring(this.code.indexOf('(function'), this.code.length - 1);
		...
		this.includes = codify_includes(this.options.includes, this.options.minify);
	...
	execute() {
		let args = execute_args(this, arguments);

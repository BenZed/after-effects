...
function find_application_dir(applications_dir) {
  ...
  let files_in_applications_dir = fs.readdirSync(applications_dir);

  for (let i = 0; i < files_in_applications_dir.length; i++) {
    let file = files_in_applications_dir[i];
    if (!file.includes('Adobe After Effects'))
      continue;

    let app_dir = path.join(applications_dir, file);
...
function find_application(application_dir) {
  ...
  let files_in_application_dir = fs.readdirSync(application_dir);

  for (let i = 0; i < files_in_application_dir.length; i++) {
    let file = files_in_application_dir[i];
...
function create_apple_script(command, sync) {
  let apple_script_file = path.join(os.tmpdir(),`ae-command-${uuid.v4()}.scpt`);
  let execute_app = determine_application(command);

  if (execute_app === null)
    throw new Error(MissingAfterEffects);

  let apple_script = `tell application "${execute_app}"
